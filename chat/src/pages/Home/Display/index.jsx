...
export default function Display() {
  ...
  const { id, name, setData } = useUser();

  useEffect(() => {
    if (data?.id) {
      if (data?.unreadMessages)
        fetch.post(`/api/chats/${data.id}/readMessages`, { id });
      setData((oldData) => {
        const currentTypeIndex = oldData.chats.findIndex(
          (array) => array.id === data?.id,
        );
        const newData = { ...oldData };
        if (newData.chats[currentTypeIndex])
          newData.chats[currentTypeIndex].unreadMessages = 0;
        return newData;
      });
    }
  }, [id, data?.unreadMessages, data?.id, setData]);
